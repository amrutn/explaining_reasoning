...
DEPTH = 3
# Specific R values requested for the X-axis
R_VALUES = [1, 4/3, 5/3, 2, 7/3, 8/3, 3]
# Specific M values requested for the curves
M_VALUES_TO_PLOT = [14, 17, 20, 23]
REPLICATES = 20
...
def run_experiment(task, mode, epochs=EPOCHS, train_samples=TRAIN_SIZE, batch_size=BATCH_SIZE, accumulation_steps=1, 
    ...
    tokenizer = train_tokenizer(task, size=min(train_samples, 5000))
    ...
    try:
        for task_def in my_tasks:
            try:
                ...
                if task_type == 'base':
                    # Equivalent to r=1
                    task_obj = TreeParityTask(branching_factor=m, depth=n, seed=seed, input_dim=INPUT_DIM)
                else:
                    task_obj = RedundantTreeParityTask(branching_factor=m, base_depth=n, depth_factor=r, seed=seed, input_dim=INPUT_DIM)
                
                hist = run_experiment(task_obj, 'reasoning', 
                                     epochs=epochs, train_samples=train_samples, batch_size=batch_size, 
                                     task_id=exp_id, gpu_id=gpu_id, pbar_pos=rank)
                ...
                tqdm.write(f"[Worker {rank}] Finished {exp_id}. Error: {error:.4f}")
...
if __name__ == "__main__":
    ...
    # Generate tasks for the plot
    for m in M_VALUES_TO_PLOT:
        for r in R_VALUES:
            for seed in SEEDS:
                # Reuse naming convention to pick up cached files
                # If r=1, we use the "base" task ID format and class
                if r == 1:
                    task_id = f"base_m{m}_n{DEPTH}_seed{seed}"
                    t_type = 'base'
                else:
                    task_id = f"over_m{m}_n{DEPTH}_r{r}_seed{seed}"
                    t_type = 'over'
    ...
    results_flat = run_task_batch(tasks, target_gpus, config_base)
    
    # Process results for plotting
    # Structure: results_data[m][r] = [errors...]
    results_data = {} 
    ...
    for item in results_flat:
        ...
        if m not in results_data:
            results_data[m] = {}
        if r not in results_data[m]:
            results_data[m][r] = []
            
        results_data[m][r].append(err)
    ...
    color_intensities = np.arange(0.6,1.01,0.4/(len(M_VALUES_TO_PLOT)-1))
    
    sorted_ms = sorted(M_VALUES_TO_PLOT)
    
    for idx, m in enumerate(sorted_ms):
        if m not in results_data:
            continue
            
        r_dict = results_data[m]
        sorted_rs = sorted(r_dict.keys())
        ...
        for r in sorted_rs:
            ...
            sem_err = np.std(errors) / np.sqrt(len(errors))
            means.append(mean_err)
            sems.append(sem_err)
            
        color = blues(color_intensities[idx])
        
        plt.plot(sorted_rs, means, '-', 
                 color=color, 
                 linewidth=2, 
                 label=f"$m={m}$", 
                 alpha=0.9, 
    ...
    plt.xlabel("Depth Factor $r$")
    plt.ylabel("Test Error")
    plt.legend(loc='upper center', frameon=False, bbox_to_anchor=(0.4,1.0), handlelength=1.5)
    plt.grid(True, linestyle=':', alpha=0.6)
    
    # Ensure X-axis ticks match the specific r values requested
    plt.xticks(R_VALUES[::2], [f"{x:.2f}" for x in R_VALUES[::2]])
    
    plt.tight_layout()
    plt.savefig("experiment_error_vs_r.pdf", dpi=300, bbox_inches='tight')
    print("Plot saved to experiment_error_vs_r.pdf")
